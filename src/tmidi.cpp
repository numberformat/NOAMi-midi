	...
	switch (iMsg)
	{
		case WM_INITDIALOG:
			...
			// Create the status bar
						hwndStatusBar = CreateStatusWindow(WS_CHILD | WS_VISIBLE, "", hDlg, 0);
			...
			j = 1;
			for (i = 0; i < controller_names_count; i++)
			{
			...
			if (LOWORD(wParam) >= IDC_T0 && LOWORD(wParam) <= IDC_T15)
			{
				...
				if (LOWORD(wParam) == IDC_T9)	// Percussion channel
				{
					if (HIWORD(wParam) == STN_CLICKED)
					{
						...
						for (i = 0; i < drum_kit_names_count; i++)
						{
							ch = strchr(drum_kit_names[i], '-');
							mii.dwTypeData = const_cast<LPSTR>(ch + 2);
							mii.cch = strlen(ch + 2);
							mii.wID = i + 2;
				...
				else	// Regular instrument channels
				{
					for (category = 0; category < 16; category++)
					{
						...
						for (instrument = 0; instrument < 8; instrument++)
						{
							...
							mii.fType = MFT_STRING;
							mii.dwTypeData = const_cast<LPSTR>(get_program_name(mii.wID - 2, 0));
							InsertMenuItem(cmenu[category], 999, TRUE, &mii);
						}
						// Then create a submenu entry
						mii.fMask = MIIM_SUBMENU | MIIM_TYPE;
						mii.fType = MFT_STRING;
						mii.dwTypeData = const_cast<LPSTR>(program_categories[category]);
...
int process_midi_event(track_header_t *th)
{
	...
	char eventname[128];
	char buf[2048];
	unsigned int i, blank, len;
	unsigned char cmd, d1, d2, d3, d4, d5;
	signed char sd1;
	unsigned int intd;
	...
	if (cmd == 0xFF)		// Meta-event
	{
		...
		if (len > (th->dataptr - th->data) + th->length)
		{
			sprintf(buf, "Meta-event %02X at track %d offset %d has length %d, "
						 ...
						 cmd, th->tracknum, static_cast<int>(cmdptr - th->data), len, th->length, ms.filename);
...
//				dprintf("Set Sequence number: %d\n", intd);
			...
			case 0x07:		// Cue point
				...
				else
				{
					...
					{
						strncpy(ms.current_text, (char *) text, sizeof(ms.current_text) - 1);
						ms.current_text[sizeof(ms.current_text) - 1] = '\0';
					}
					...
					case 0x07:	// SYSEX data
						...
						if (len > (th->dataptr - th->data) + th->length)
						{
							sprintf(buf, "Sysex data at track %d offset %d has length %d, "
										 "which would exceed track length of %d.\n\n"
										 "Conclusion: %s is corrupt.", 
										 th->tracknum, static_cast<int>(cmdptr - th->data), len, th->length, ms.filename);
							MessageBox(hwndApp, buf, "TMIDI Read Error", MB_ICONERROR);
							return 1;
						}
						sysexptr = th->dataptr;
...
for (i = 0; i < sysex_manufacturer_names_count; i++)
		if (atoi(sysex_manufacturer_names[i]) == id)
			break;
if (i == sysex_manufacturer_names_count)
...
{
	int hdc_specified = 0;
	int i, j, note, vol, x, y, r = 0, g = 0, b = 0, h, pb;
	HPEN hpen, holdpen;
	channel_state_t *c;
	...
	switch (iMsg)
	{
		...
		case WM_COMMAND:
			if (HIWORD(wParam) == LBN_DBLCLK)
			{
				...
				if (i != LB_ERR)
				{
					LONG_PTR itemData = SendMessage((HWND) lParam, LB_GETITEMDATA, i, 0);
					if (itemData != LB_ERR)
					{
						t = (midi_text_t *) itemData;
...
{
	LV_COLUMN lvc;
	const char * const columns[NUM_TRACK_COLUMNS] = { 
		"Track", 
		"Bytes", 
		"Event", 
		"Events", 
	...
	for (i = 0; i < NUM_TRACK_COLUMNS; i++)
	{
		...
		lvc.pszText = const_cast<LPSTR>(columns[i]);
		if (i == 5 || i == 6 || i == 9)
			lvc.fmt = LVCFMT_LEFT;
		else
...
void SetCachedLVItem(HWND hwndLV, int iItem, int column, signed int &lastval, const char *str)
{
	...
	{
		lastval = h;
		ListView_SetItemText(hwndLV, iItem, column, const_cast<LPSTR>(str ? str : ""));
	}
}
...
{
	LV_COLUMN lvc;
	const char * const columns[NUM_CHANNEL_COLUMNS] = { 
		"Channel", 
		"Program", 
		"Bank", 
		"Note", 
...
{ 
    ...
    // control. 
    if (!EnumChildWindows(hwndApp, EnumChildProc, 0)) 
        return FALSE; 
...
{ 
    ...
    if ((((LPNMHDR) lParam)->code) == TTN_NEEDTEXT) { 
		...
		{
            case IDC_MIDI_OUT:
                lpttt->lpszText = const_cast<LPSTR>("Bite me!");
                break;
        }
    }
	return;
...
const char *stristr(const char *src, const char *target)
{
	const char *found = NULL;	// pointer to beginning of matched string
	const char *sp;			// search pointer
	const char *tp;			// target pointer
	...
	while (*src)
	{
		...
		while (*sp && *tp)
		{
			...
			if (tolower(static_cast<unsigned char>(*sp)) == tolower(static_cast<unsigned char>(*tp)))
			{
		...
		case MIDI_STANDARD_MT32:
		{
			...
			switch (g_mt32_state.PatchGroup(program))
			{
				...
				case 3:
					...
					return name;
				default:
					return mt32_program_names[patch_program];
			}
		}
		default:
			if (bank == 0 || bank == 255)
...
for (i = 0; i < controller_names_count; i++)
	{
		if (strcmp(controller_names[i], "Unknown controller"))
		{
			...
			mii.wID = i + 1024;
			mii.fType = MFT_STRING;
			mii.dwTypeData = const_cast<LPSTR>(controller_names[i]);
	...
	{
		for (i = 0; i < common_controller_names_count; i++)
			if (atoi(common_controller_names[i]) == c->last_controller)
				break;
		if (i == common_controller_names_count)
		{
			mii.fMask = MIIM_ID | MIIM_TYPE | MIIM_STATE;
			mii.fState = MFS_DEFAULT;
